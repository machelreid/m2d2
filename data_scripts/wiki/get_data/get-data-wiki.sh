...
# aria2c -x16 --dir $WIKI_PATH/bz2/ $WIKI_DUMP_LINK 
...
if [ ! -f $WIKI_PATH/txt/$lg.all ]; then
  python -m wikiextractor.WikiExtractor $WIKI_PATH/bz2/$WIKI_DUMP_NAME -o $HOME/storage/wiki_dumps_json
 #  \
 #  | sed "/^\s*\$/d" \
 #  | sed '/<doc id=/c\\n' \
 #  | sed '/<\/doc>/c\\n' \
